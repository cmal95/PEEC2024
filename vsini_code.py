...
#MODELS_PATH  = ""
MOOG_PATH     = "/home/spec/Programs/MOOG2019/./"  # otherwise write your full path to MOOGSILENT here
MODELS_PATH   = "/home/spec/Programs/interpol_models/./"
LINELIST_PATH = 'linelist/'
SPECTRA_PATH = 'Spectra_rv/'
...
#                    'step': 0.001}
    vrot_info = {'parname': 'vrot', 'value': 5, 'fixed': 0, 'limited': [1, 1], 'limits': [0.1, 50], 'mpside': 2,
                    'step': 0.001}
    parinfo = [vrot_info]
...
def get_vsini(star, spectrum, teff, feh, vtur, logg, ldc, instr_broad, fe_intervals):
    ...
    interp_function = interp1d(obs_lambda_full_spectrum, obs_data_full_spectrum, fill_value="extrapolate")
...
def manual_test(star, spectrum, teff, feh, vtur, logg, ldc, instr_broad, fe_intervals, vrot_test):
    ...
    plt.savefig('/home/spec/WORK/PEEC2024/manual_tests/'+star+'.jpg')
    plt.show()
...
def interpolation_function(T,log,feh):
    ...
    if feh==0.51:
        return 0.613
    #values defined through approximation
    if feh>=0.5:
        feh=0.49999999999
    if feh<=-0.5:
        feh=-0.49999999999
...
def main():
    ...
    directory="exp_stars_information.csv" #list of information about the spectra
    ...
    #spectrum = "/home/pedro/OneDrive/Documentos/codes/Espetros/" + directory
        ...
        manual_test(star, SPECTRA_PATH+fits_directory[i], teff, feh, vtur, logg, ldc, instr_broad, fe_intervals,3.0)
        return
...
#With Error propagation
        ...
        print ('results:', '\nstar=',star, '\nteff=',teff, '\nlogg=',logg,'\nfeh=', feh, '\nSpectra Path=',SPECTRA_PATH+fits_directory[i], '\nvrot=',vrot, '\nvrot_err=',vrot_err, '\nvmac=',vmac, '\nstatus=',status,'\nvsini_final_err=', vsini_final_err)
        #print ('results:','\nstar=', star,'\nteff=', teff,'\nlogg=', logg,'\nfeh=', feh,'\nsnr=', snr,'\nspectrum=', spectrum,'\nvrot=', vrot,'\nvrot_err=', vrot_err,'\nvmac=', vmac,'\nstatus=', status, '\nvsini_final_err=', vsini_final_err)
    #saving the results in a csv file
        List=[star,instr_broad,teff,logg,feh,vrot, vrot_err, vmac, status, vsini_final_err,date]
    
        with open('newtriestocorrect.csv', 'a') as f_object:
